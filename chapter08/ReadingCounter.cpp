...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdParam, int nCmdShow) {
	g_hInst = hInstance;
	DialogBox(g_hInst, MAKEINTRESOURCE(IDD_DIALOG1), HWND_DESKTOP, ReadingDlgProc);
	return 0;
}
int PageStart, PageEnd, NowPage;
int PageSec, NowSec;
enum tag_Status{WAIT,RUN,PAUSE}Status;
	...
	case WM_COMMAND:
		...
		case IDC_BTNSTART:
			...
			if (PageStart == 0 || PageEnd == 0 || PageStart >= PageEnd) {
				MessageBox(hDlg, TEXT("페이지 번호를 다시 입력하십시오."), TEXT("알림"), MB_OK);
				return TRUE;
			}
			NowPage = PageStart;
