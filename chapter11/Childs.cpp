...
LRESULT CALLBACK ChildLeftProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE:
		...
		SendMessage(hList, LB_ADDSTRING, 0, (LPARAM)TEXT("리스트박스"));
		hEdit = CreateWindow(TEXT("edit"), NULL, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
			210, 10, 80, 25, hWnd, (HMENU)1, g_hInst, NULL);
		hBtn = CreateWindow(TEXT("button"), NULL, WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
...
LRESULT CALLBACK ChildTopProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	HDC hdc;
	PAINTSTRUCT ps;
	TCHAR *Mes = TEXT("이 윈도우는 차일드를 가지고 있지 않습니다.");
...
LRESULT CALLBACK ChildBottomProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	{
	case WM_CREATE:
		hEdit = CreateWindow(TEXT("edit"), NULL, WS_CHILD | WS_VISIBLE | WS_BORDER | ES_MULTILINE | WS_VSCROLL,
